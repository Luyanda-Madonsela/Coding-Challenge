...
class Task {
    ...
    constructor(id: number, title: string, description: string, completed: string) {
      this.id = id;
      this.title = title;
...
//-------------------------------------------------------------------------------------
// TaskManager class defines the methods and attributes of the TaskManager object. 
class TaskManager {
    tasks: Task[];
    taskId: number;
 ...
 // addTask method takes a title and a description, creates a task object to store all tha values. Then the object is then stored in the array.
    addTask(title:string, description:string) {
      const task = new Task(this.taskId, title, description,' '); // Instantiates a new task object and gives it the parameters of the addTask method
...
//-------------------------------------------------------------------------------------
  ...
  5. Exit`);
      ...
      const answer = await new Promise<string>((resolve) => {
        scanner.question('Enter your choice: ', (input: string) => {
          resolve(input);
        });
      });
      ...
      switch (answer) {
          ...
          case '1':
                ...
                const title = await new Promise<string>((resolve) => {
                  scanner.question('Enter task title: ', (input: string) => {
                  resolve(input);
                  });
                });
                const description = await new Promise<string>((resolve) => {
                  scanner.question('Enter task description: ', (input: string) => {
                  resolve(input);
                  });
                });
                taskManager.addTask(title, description); // Calling method to add a task to array
                break;

          case '2':
                console.log('Mark a task as completed:');
                const taskId = await new Promise<number>((resolve) => {
                  scanner.question('Enter the task ID to mark as completed: ', (input: string) => {
          ...
          case '4':
                ...
                const deleteTaskId = await new Promise<number>((resolve) => {
                  scanner.question('Enter the task ID to delete: ', (input: string) => {
                  resolve(parseInt(input));
                  });
                });
